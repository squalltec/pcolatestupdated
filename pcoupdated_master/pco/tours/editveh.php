	...
	$imgs=$_FILES['imagescars']['name'];
        ...
        if (!empty(array_filter($imgs))) {
            ...
            foreach($imgs as $id=>$val){
                ...
                if(!empty($sqlVal)) {
                    $insert = $conn->query("INSERT INTO carsimages (hotel, country,city,brand,model,registration,color,image) VALUES ('$hot', '$cou', '$cit', '$bra', '$mod', '$reg', '$col','$fileName')");
...
$sql1="SELECT * FROM carsimages WHERE registration='$rrgg'";
...
if ($result1->num_rows > 0) {
  ...
  while($row1 = $result1->fetch_assoc()) {
   ...
   <img style="position: relative; width:100px;" src='../Car Images/<?php echo $h;?>/<?php echo $b;?>/<?php echo $m;?>/<?php echo $row1['image']; ?>'>
  ...
  <input type='submit' value='Upload' class='btn btn-primary' style='float:right;' name='updateimages'>
  </div>
  </div></div>
