...
foreach($colors as $value){
	...
	mkdir("../RentacarCar Images/".$hotel."/".$brandd."/".$roomtype."/".$year, 0755, true);	
		
		 $uploadsDir = "../RentacarCar Images/".$hotel."/".$brandd."/".$roomtype."/".$year."/";
	...
	$imgs=$_FILES['filesfacilitiess'.$nn]['name'];
        ...
        if (!empty(array_filter($imgs))) {
            ...
            foreach($imgs as $id=>$val){
                ...
                if(!empty($sqlVal)) {
                    $insert = $conn->query("INSERT INTO rentacarcarsimages (hotel, country,city,brand,model,color,image,year) VALUES ('$hotel', '$country', '$city', '$brandd', '$roomtype', '$value','$fileName','$year')");
...
<form action="#" method="post" enctype="multipart/form-data">
  ...
  <h2 style='margin-top:-60px;'><?php echo $brandd.' '.$roomtype.' '.'('.$year.')';?> </h2>  
