...
foreach ($brand as $value) {
    ...
    for($i=0;$i<$numberofcars[$nn]; $i++){
        ...
        $uuid[$i] = vsprintf('%s%s%s%s%s%s%s%s', str_split(bin2hex($data), 4));
        $availableStaus[$i] = 1;
    
    }
    $uuidString = implode(',',$uuid); 

    $availableStausString = implode(',',$availableStaus);


$sql = "INSERT INTO rentacarInventorydatabase (hotel,country,city,brand,model,year,numberofcars,transmission,uuid,availableforbooking)
           VALUES ('$hotel','$country','$city','$value','$model[$nn]','$year[$nn]','$numberofcars[$nn]','$trans[$nn]','$uuidString','$availableStausString')";
