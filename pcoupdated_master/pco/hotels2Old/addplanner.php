...
if ($resulta === TRUE) {
 ...
 mkdir("../Banquet Images/".$country."/".$city."/".$hotel."/".$venue, 0755, true);	

 $uploadsDir = "../Banquet Images/".$country."/".$city."/".$hotel."/".$venue."/";
 $allowedFileType = array('jpg','png','jpeg');
		
	$img=array();
	
	$imgs=$_FILES['images'.$nn]['name'];
        ...
        if (!empty(array_filter($imgs))) {
            ...
            foreach($imgs as $id=>$val){
                ...
                $fileName        = $_FILES['images'.$nn]['name'][$id];
                $tempLocation    = $_FILES['images'.$nn]['tmp_name'][$id];
                ...
                if(!empty($sqlVal)) {
                    $insert = $conn->query("INSERT INTO meetingplannerimages (hotel, country,city,venue,image) VALUES ('$hotel', '$country', '$city','$venue','$fileName')");
        ...
        <div class="card">
					<div class="card-body">
						<div class="table-responsive">
							...
							<td><input class='form-control' name='images0[]' type='file' accept='image/*' multiple ></td>
...
<input style='display:none;' id='imgcounter' value='0'>
...
function myFunction2() {
    ...
    var imagescounter = parseInt(imgcounter)+1;
    
    document.getElementById('imgcounter').value=imagescounter;
...
var yc12 = document.createElement("INPUT");

yc12.setAttribute("class", "form-control");
yc12.setAttribute("Name", "images"+imagescounter+"[]");
yc12.setAttribute("type", "file");
yc12.setAttribute("multiple", "true");
yc12.setAttribute("accept", "image/*");
